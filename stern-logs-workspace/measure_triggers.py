...
def main() -> int:
    ...
    eval_set = json.loads((REPO / "stern-logs-workspace" / "eval_set.json").read_text())
    ...
    (REPO / "stern-logs-workspace" / "trigger_results.json").write_text(
        json.dumps({"summary": summary, "results": out}, indent=2, ensure_ascii=False)
    )
